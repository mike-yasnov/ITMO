...
public class Ask {
    ...
    public static Product askProduct(Console console, int id) throws AskBreak {
        try {
            String name;
            while (true) {
                console.print("name: ");
            ...
            var price = askLong(console, "price");
            var partNumber = askStringInRange(console, "partNumber", 27, 42, true);
            var manufactureCost = askInteger(console, "manufactureCost");

            var unitOfMeasure = askUnitOfMeasure(console);
            var owner = askOwner(console);
            return new Product(id, name, coordinates, java.time.ZonedDateTime.now(), price, partNumber, manufactureCost, unitOfMeasure, owner);
    ...
    public static Long askLong(Console console, String fieldName) throws AskBreak {
        try {
            long value;
            while (true) {
    ...
    public static Integer askInteger(Console console, String fieldName) throws AskBreak {
        try {
            int value;
            while (true) {
                console.print(fieldName + ": ");
    ...
    public static Person askOwner(Console console) throws AskBreak {
        try {
            ...
            while (true) {
                console.print("owner.passportID: ");
                var line = console.readln().trim();
                if (line.equals("exit")) throw new AskBreak();
                if (!line.isEmpty()) {
                    try {
                        ownerPassportID = line;
                        break;
                    } catch (NumberFormatException e) {
                        console.printError("Неверный формат числа");
                    }
            ...
            return new Person(ownerName, ownerPassportID, eyeColor, hairColor, nationality, ownerLocation);
        } catch (NoSuchElementException | IllegalStateException e) {
            console.printError("Ошибка чтения");
            return null;
        }
