    ...
    public boolean insertProduct(Product product) {
        try {
            ...
            if (owner != null) {
                ...
                stmt.setString(2, owner.getPassportID());
                ...
                stmt.setDate(4, java.sql.Date.valueOf(String.valueOf(product.getCreationDate())));
                ...
                stmt.setString(9, String.valueOf(product.getOwner()));
    ...
    synchronized public void loadCollection(TreeSet<Product> collection) {
        try {
            ...
            while (rs.next()) {
                ...
                ZonedDateTime creationDate = offsetDateTime.toZonedDateTime();
                ...
                if (ownerId != null) {
                    ...
                    String passportID = rs.getString(14);
                    ...
                    int locationY = rs.getInt(19);
                    String locationName = rs.getString(20);
                    owner = new Person(ownerName, passportID, eyeColor, hairColor, nationality, new Location(locationX, locationY, locationName));
                }
                Product product = new Product(productId, productName, new Coordinates(x, y), creationDate, price, partNumber, cost, unitOfMeasure, owner);
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (ownerId != null) {
                    ...
                    String locationName = rs.getString(20);
                    owner = new Person(ownerName, passportID, eyeColor, hairColor, nationality, new Location(locationX, (int) locationY, locationName));
                }
                return new Product(productId, productName, new Coordinates(x, y), creationDate, price, partNumber, cost, unitOfMeasure, owner);
            } else return null;
    ...
    public boolean update(int id, Product newProduct) {
        try {
            if (newProduct.getOwner() != null) {
                ...
                st.setDate(4, java.sql.Date.valueOf(String.valueOf(newProduct.getCreationDate())));
                ...
                st.setString(9, String.valueOf(newProduct.getOwner()));
                st.setInt(10, id);
                ResultSet rs = st.executeQuery();
                if (rs.next()) {
                    stmt = "update owners set name=?, passport_id=?, eyecolor=?, haircolor=?, nationality=? where id=? returning location";
                    st = conn.prepareStatement(stmt);
                    st.setString(1, newProduct.getOwner().getName());
                    st.setString(2, newProduct.getOwner().getPassportID());
